...
export class AccountsActivityActionViewItem extends AbstractGlobalActivityActionViewItem {
	...
	protected override async resolveMainMenuActions(accountsMenu: IMenu, disposables: DisposableStore): Promise<IAction[]> {
		...
		// The only account is the GitHub account of the page
		const account = this.githubAccountService.account;
		if (!account) {
			return [toAction({ id: 'vsprocessing.github.signIn', label: localize('signInWithGitHub', "Sign in with GitHub"), run: () => this.githubAccountService.signIn() })];
		}
		return [
			disposables.add(new Action('vsprocessing.github.account', account.label, undefined, false)),
			new Separator(),
			toAction({ id: 'vsprocessing.github.signOut', label: localize('signOut', "Sign Out"), run: () => this.githubAccountService.signOut() }),
		];
	}
