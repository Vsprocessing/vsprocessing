	...
	private async _createExtHostInitData(): Promise<IExtensionHostInitData> {
		...
		return {
			...
			environment: {
				...
				// Global storage belongs to the current profile: each signed in account keeps its own
				// (such as its virtual folders), and the guest profile's only lives for the session
